...
class SetuInventoryFSNAnalysisReport(models.TransientModel):
    ...
    def get_inventory_fsn_analysis_report_data(self, for_reporting=False):
        """
        :return:
        """
        ...
        vals_list = {'company_ids': [(4, id) for id in list(company_ids)],
        ...
        turn_over_ratio_vals = self.env['setu.inventory.turnover.analysis.report'].create(vals_list)
        if self._context.get('report_company_wise', False):
            turn_over_ratio_vals.with_context(report_company_wise=True).get_inventory_turnover_ratio_data()
        else:
            turn_over_ratio_vals.get_inventory_turnover_ratio_data()
        ...
        query = """
                Select * from get_inventory_fsn_analysis_report('%s','%s','%s','%s','%s','%s', '%s','%d')
            """%(company_ids, products, category_ids, warehouses, start_date, end_date, self.stock_movement_type,self.id)
        # print(query)
        if self._context.get('report_company_wise', False):
            query = """
                            Select * from get_inventory_fsn_analysis_report_company_vise('%s','%s','%s','%s','%s','%s', '%s','%d')
                        """ % (
            company_ids, products, category_ids, warehouses, start_date, end_date, self.stock_movement_type,self.id)
        self._cr.execute(query)
        stock_data = self._cr.dictfetchall()
        if for_reporting:
            self._cr.execute(f"""select * from setu_inventory_fsn_analysis_bi_report where wizard_id={self.id}""")
            stock_data = self._cr.dictfetchall()
            return stock_data
        return stock_data
    ...
    def download_report(self):
        ...
        stock_data = self.get_inventory_fsn_analysis_report_data(for_reporting=True)
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.fsn.analysis.report&field=stock_file_data&id=%s&filename=%s'%(self.id, file_name),
            'target': 'self',
        }
