...
class SetuInventoryFSNXYZAnalysisReport(models.TransientModel):
    ...
    def get_inventory_fsn_xyz_analysis_report_data(self, for_reporting=False):
        """
        :return:
        """
        ...
        fsn_vals.with_context(report_company_wise=True).get_inventory_fsn_analysis_report_data()
        xyz_vals_list = {'company_ids': [(4, id) for id in list(company_ids)],
                     'product_ids': [(4, p_id) for p_id in list(products)] if products else [],
                     'product_category_ids': [(4, p_c_id) for p_c_id in list(category_ids)] if category_ids else []}
        xyz_vals = self.env['setu.inventory.xyz.analysis.report'].create(xyz_vals_list)
        xyz_vals.get_inventory_xyz_analysis_report_data()
        # get_products_overstock_data(company_ids, product_ids, category_ids, warehouse_ids, start_date, end_date, advance_stock_days)
        query = """
                Select * from get_inventory_fsn_xyz_analysis_report('%s','%s','%s','%s','%s','%s', '%s', '%s','%d')
            """%(company_ids, products, category_ids, warehouses, start_date, end_date, self.stock_movement_type, self.inventory_analysis_type, self.id)
        # print(query)
        self._cr.execute(query)
        stock_data = self._cr.dictfetchall()
        if for_reporting:
            self._cr.execute(f"""select * from setu_inventory_fsn_xyz_analysis_bi_report where wizard_id={self.id}""")
            stock_data = self._cr.dictfetchall()
            return stock_data
        return  stock_data
    ...
    def download_report(self):
        ...
        stock_data = self.get_inventory_fsn_xyz_analysis_report_data(for_reporting=True)
