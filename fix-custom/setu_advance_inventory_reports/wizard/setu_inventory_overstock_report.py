...
class SetuInventoryOverstockReport(models.TransientModel):
    ...
    def get_overstock_report_data(self, for_reporting=False):
        """
        [
           {
              "company_id":1,
        ...
        query = """
                Select * from get_products_overstock_data('%s','%s','%s','%s','%s','%s', '%s','%d')
        ...
        if for_reporting:
            self._cr.execute(f"""select * from setu_inventory_overstock_bi_report where wizard_id={self.id}""")
            stock_data = self._cr.dictfetchall()
            return stock_data
        return  stock_data
    ...
    def download_report(self):
        ...
        stock_data = self.get_overstock_report_data(for_reporting=True)
        ...
        return {
            ...
            'url': '/web/binary/setu_air_download_document?model=setu.inventory.overstock.report&field=stock_file_data&id=%s&filename=%s'%(self.id, file_name),
            'target': 'self',
        }
