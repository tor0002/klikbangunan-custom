...
class SetuInventoryTurnoverAnalysisReport(models.TransientModel):
    ...
    def get_inventory_turnover_ratio_data(self, for_reporting=False):
        """
        ...
        query = """
                Select * from get_inventory_turnover_ratio_data('%s','%s','%s','%s','%s','%s','%d')
            """ % (company_ids, products, category_ids, warehouses, start_date, end_date,self.id)
        if self._context.get('report_company_wise', False):
            query = """
                            Select * from get_inventory_turnover_ratio_data_company_vise('%s','%s','%s','%s','%s','%s','%d')
                        """ % (company_ids, products, category_ids, warehouses, start_date, end_date, self.id)
        #print(query)
        self._cr.execute(query)
        stock_data = self._cr.dictfetchall()
        if for_reporting:
            self._cr.execute(f"""select * from setu_inventory_turnover_analysis_bi_report where wizard_id={self.id}""")
            stock_data = self._cr.dictfetchall()
            return stock_data
        return stock_data
    ...
    def download_report(self):
        ...
        stock_data = self.get_inventory_turnover_ratio_data(for_reporting=True)
    ...
    def download_report_in_listview(self):
        ...
        tree_view_id = self.env.ref(
            'setu_advance_inventory_reports.setu_inventory_turnover_ratio_analysis_bi_report_tree').id
