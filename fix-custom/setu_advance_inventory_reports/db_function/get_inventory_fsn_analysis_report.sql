...
DROP FUNCTION IF EXISTS public.get_inventory_fsn_analysis_report(integer[], integer[], integer[], integer[], date, date, text,integer);
...
BEGIN
    ...
    Insert into setu_inventory_fsn_analysis_bi_report(company_id,product_id,product_category_id,warehouse_id,opening_stock,closing_stock,average_stock,sales,turnover_ratio,stock_movement,company_name,product_name,category_name,warehouse_name,wizard_id)
    select F.company_id,F.product_id,F.product_category_id,F.warehouse_id,
    F.opening_stock,F.closing_stock,F.average_stock,F.sales,F.turnover_ratio,F.stock_movement,F.company_name,F.product_name,F.category_name,F.warehouse_name,fsn_wizard_id from(
    Select * From
    (
        Select
            t_data.*,
