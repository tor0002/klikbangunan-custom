...
class StockPickingBatch(models.Model):
    ...
    def _check_weight_limit(self):
        for batch in self:
            if batch.x_cars == 'Carry' and batch.x_sum_total_weight > 1000:
                raise UserError('Berat total tidak boleh melebihi 500 untuk Carry')
            elif batch.x_cars == 'Engkel' and batch.x_sum_total_weight > 3000:
                raise UserError('Berat total tidak boleh melebihi 1000 untuk Engkel')
            elif batch.x_cars == 'Traga' and batch.x_sum_total_weight > 2950:
                raise UserError('Berat total tidak boleh melebihi 2950 untuk Traga')
