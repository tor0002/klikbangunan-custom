...
class ReportKsiPostReportReportPos(models.Model):
    _name = 'report.ksi_post_report.report_pos'
    ...
    def generate_xlsx_report(self, workbook, data, obj):
        ...
        sheet.merge_range(1, 0, 1, 1, "PRODUCT REPORT")
        ...
        self.env.cr.execute(
            ...
            WITH product_per_month AS (
                SELECT 
                    ...
                    date_trunc('month', timezone('Asia/Bangkok', timezone('UTC',rpo.date))::timestamp) AS month_filter
                FROM 
                    report_pos_order AS rpo
                LEFT JOIN
                    product_product AS pp ON pp.id = rpo.product_id
                LEFT JOIN
                    product_template AS pt ON pt.id = pp.product_tmpl_id
                WHERE ("rpo"."company_id" IS NULL  
                    OR ("rpo"."company_id" in {company}))
                GROUP BY
                    product_id,
                    product_name,
                    pt.priority,
            ...
            FROM product_month
            """.format(month=obj.month, year=obj.year, company=tuple_str_company)
        )
